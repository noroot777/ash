...
**体积有硬闸**：`.githooks/pre-commit` 棘轮式拦下「把根文件改大且超 8192 字节」的提交（只拦增长、不拦存量）。**新克隆后需执行一次 `git config core.hooksPath .githooks`**。想加一条就先搬走一条，并在交付总结里报一句根文件净 +N/−N 字节。

理由：加一条只要 2 分钟、当场生效、还显得周到，账单却落在下一个 agent 的入场费和下一次并行冲突上——产生成本的人不付账，所以要把账单送回 commit 那一刻，并让「减」和「加」一样可见（膨胀史与三笔账见 `docs/incidents.md`）。只写在文档里的自我约束拦不住任何一次追加，只有做进系统的约束才对所有 CLI 品牌的 agent 一视同仁。
