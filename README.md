...
侧栏底部状态栏「收起」旁的 **助手** 入口进入聊天页，再从顶部工作模式栏「聊天」右侧打开助手，即可回答使用问题。首次打开时选择已配置的智能体、模型与智能水平，之后无需 `@` 即可发问；还没有项目时也能使用。助手对话和执行器选择跨项目保留，明确委派的工作落在发送消息时所选的项目。

- 忘记任务名称时，描述大概做过什么，例如「帮我找以前处理登录认证的任务」。助手会拆出关键词检索当前用户可见项目中的任务标题、正文和会话，给出可打开的结果；归档任务可只读查看。
- 描述想要的起手式，例如「写完代码跑构建和测试，等我确认再合并」。助手会生成可查看各站配置的草案，点击「保存起手式」后进入起手式库，可继续编辑；保存不会立即运行任务或改变默认值。
- 明确委派其它项目工作时，助手会创建并启动关联任务，在对话中显示任务卡。停止助手回复不会停止已经创建的任务。对话、草案和停止状态在刷新后保留，`/clear` 可重置后续上下文。
...
npm run test:web-gate                 # 上面那道 pre-push 闸自己的回归
