...
- 已建好指向该仓库的项目
- 安装了对应的 agent CLI
- **在它的「接力来源」里批准过本机**（首次配对：每台 Ash 有一对自己的密钥，指纹核对一次即可）
...
| 目标机换了机器 | 指纹对不上，**打包之前**就拒绝——不会把仓库和对话历史发错地方 |
| 对端没批准本机 | 预检如实报出，接力按钮按住不放；到对端点一下批准即可 |
...
> 跨机器**接力**是例外：它走独立的机器身份校验（一机一对 ed25519 密钥、双向指纹核对、逐请求签名），见 [docs/handoff.md](docs/handoff.md)。但传输仍是明文 HTTP，不解决窃听。
