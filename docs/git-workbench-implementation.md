...
## 按行丢弃语义提示修复（2026-09-16）

本轮仅处理审查 `1Ytj0A98L0hh/round-2` 的 R2-1，采用报告给出的文案方案。按行丢弃继续精确执行勾选的差异行，不自动扩大选择范围。确认框将「仅还原勾选的 N 行改动」改为「逐行撤销勾选的 N 行差异」，并与选择工具条共用说明：所选＋行从文件中删除，所选−行恢复；修改只选＋行时，被替换的原始行不会恢复，只选−行时新增内容会保留；完整还原修改需同时勾选对应的 − / + 行。暂存、整块丢弃及后端行为不变。

`npm -w web run build` 通过，包含类型与前端约定检查，仅有既有打包体积提示。后端 `test-git-workbench-partial-discard.ts` 通过，保留确认词、版本和差异校验，以及 HEAD/暂存区/其他改动保护与无末尾换行用例。本轮没有改动报告已确认为存量的测试等待问题，也未重跑完整旧测试链。

扩展后的 `npm -w web run test:git-workbench-discard-menu` 通过。真实修改型差异分别验证仅勾＋行（删除新增内容、不恢复原始行）、仅勾−行（恢复原始行、保留新增内容）、同时勾两侧（文件完整还原）；实际请求行号与勾选一致，磁盘结果与提示一致。原有纯新增行丢弃、取消和错误确认、清除选择后整块丢弃，以及弹层动态增减回归均通过。已人工核对桌面与 390px 的提示、确认框截图，文字和按钮完整可见，窄屏无横向溢出。脚本语法与 `git diff --check` 通过，修改的代码文件均少于 700 行。

本轮重新探测 Chrome 扩展，仍返回 `unsupported Codex auth method: apikey` 且浏览器列表为空，因此使用独立临时 profile 的无头 Chromium。没有接管普通标签、激活 Chrome 或使用有头模式；浏览器、Vite、独立测试后端及临时 Git 仓库已清理，没有访问正在运行的 `:4317` 实例。截图、实际请求与文件结果、浏览器通过记录及扩展失败原文保存在 `/Users/fjh/code/harness/data/runs/Y_sgd0mNlRXp/review-r2-1-20260916/`。

## 特殊前缀差异行修复（2026-09-16）

本轮仅修复审查 `1Ytj0A98L0hh/round-3` 的 R3-1。`DiffView` 已进入改动块的行只按首字符 `+/-` 判断新增或删除，不再把内容中的 `---`、`+++` 当成文件头。路径头解析仅在改动块之外生效，因此实际内容 `++ b/example.txt` 也会作为新增行显示。历史等只读视图复用相同显示逻辑。

服务端部分补丁校验改为拒绝首个改动块之前的文件头行号，并接受块内的特殊前缀内容。原有确认词、状态与差异核对、无末尾换行限制、权限与索引保护继续生效。本轮没有修改上一轮按行丢弃的语义说明或既有测试等待问题。

`server/web build` 均通过，包含类型与前端约定检查，只有既有打包体积提示；后端完整 `test:git-workbench` 六套通过。新增真实仓库样本覆盖被删的 `---`、`-- SQL comment`、新增的 `++counter;`，以及 `-- a/example.txt` 改为 `++ b/example.txt`，每种均验证整块暂存、取消暂存、丢弃和最终磁盘内容，同时核对 HEAD、已有暂存内容、其他改动与未跟踪文件保留。三种部分操作均验证真实文件头行号被拒绝。

新增 `npm -w web run test:git-workbench-diff-prefixes` 已接入工作台总测试入口，专项执行通过。真实页面验证特殊前缀行的符号、红绿背景、可选择状态、单行暂存与取消暂存，以及新增和删除块的完整暂存/取消暂存/丢弃；历史只读视图也显示正确。既有 `test:git-workbench-discard-menu` 通过，保留前两轮的选择范围、丢弃提示和弹层定位回归。本轮未重跑完整旧浏览器测试链。已人工核对新增、删除及历史截图；脚本语法与 `git diff --check` 通过，修改代码文件均少于 700 行。

本轮 Chrome 扩展重探仍返回 `unsupported Codex auth method: apikey`，浏览器列表为空，因此按顺序降级到独立临时 profile 的无头 Chromium。没有接管普通标签、激活 Chrome 或使用有头模式；独立浏览器、Vite、测试后端和临时仓库已清理，未访问正在运行的 `:4317` 实例。构建、后端日志、浏览器截图与请求行号、扩展错误原文保存在 `/Users/fjh/code/harness/data/runs/Y_sgd0mNlRXp/review-r3-1-20260916/`，既有丢弃回归的证据在其 `discard-menu/` 子目录。

## 差异勾选保留与删除文件标题修复（2026-09-16）

本轮仅修复审查 `1Ytj0A98L0hh/round-4` 的 R4-1、R4-2。差异缓存按项目、工作树、文件路径及暂存来源关联，同一选择刷新时保留已有差异；组件的 key 不随整仓库版本变化，因此返回相同差异时保留勾选。刷新期间差异写操作禁用，响应内容真的变化才重新挂载并清空勾选；切换文件或来源时立即隐藏不匹配的旧差异，过时响应仍被丢弃。

历史多文件差异标题同时读取 `---` 旧路径与 `+++` 新路径，新增侧为 `/dev/null` 时回退到同一文件的旧路径。路径头解析仍限定在改动块之外，保留上一轮对形似文件头的真实内容的修复。

`npm -w web run build` 通过，包含类型与前端约定检查，只有既有打包体积提示。新增 `test:git-workbench-refresh-paths` 已挂入工作台总测试入口；使用独立真实 Git 仓库验证两个无变化轮询、无关文件被外部修改、页面暂存另一个文件时保留当前勾选，同时检查当前差异变化、切文件和切暂存来源后清空勾选。延迟响应场景检查旧差异可读但不可写，过时响应不覆盖新选择；注入读取失败时没有旧的行操作入口。真实历史提交的两个删除文件、修改文件与新增文件均显示正确路径。

新增专项与既有 `test:git-workbench-diff-prefixes` 均通过；过时响应断言等待响应完整结束和页面渲染后执行。已人工核对刷新期间、暂存其他文件后及历史文件名截图。脚本语法与 `git diff --check` 通过，修改的代码文件均少于 700 行。本轮未修改后端，也未重跑完整旧浏览器测试链。

本轮重新探测 Chrome 扩展，返回 `unsupported Codex auth method: apikey` 且浏览器列表为空，因此降级到独立临时 profile 的无头 Chromium。没有接管普通标签、激活 Chrome 或使用有头模式；浏览器、Vite、测试后端和临时仓库已清理，未访问正在运行的 `:4317` 实例。构建日志、浏览器截图与量化结果、扩展失败原文保存在 `/Users/fjh/code/harness/data/runs/Y_sgd0mNlRXp/review-r4-20260916/`，上一轮前缀回归的证据在其 `diff-prefixes/` 子目录。

## 已暂存侧逐行说明与刷新期间勾选修复（2026-09-16）

本轮仅处理审查 `1Ytj0A98L0hh/round-5` 的 R5-1、R5-2。差异选择接口显式传入暂存来源，已暂存侧勾选后也显示逐行说明：所选 `+` 行从暂存区删除，所选 `-` 行恢复到暂存区；修改只选一侧时，暂存区会留下待提交的删除或新增内容，完整取消这处修改的暂存需勾选两侧。说明同时明确工作区文件保持不变，后端行为不变。

差异控件将禁用属性改为 `actionsDisabled`，只限制按行及按块的实际写入按钮。刷新期间行勾选、块勾选、清除选择可继续操作，并显示「正在刷新差异，可继续勾选」。内容真正变化时仍清空勾选，写操作的新鲜度、权限与忙碌状态限制保留。

`npm -w web run build` 通过，包含类型与前端约定检查，仅有既有打包体积提示。扩展后的 `test:git-workbench-refresh-paths` 通过：延迟刷新期间可勾选行和块、清除选择，实际写入按钮保持禁用，本地选择不发送写请求；相同差异保留最终勾选，内容变化仍清空。已暂存侧分别验证只选 `+`、只选 `-`、同时选两侧的实际请求与索引内容，工作区、HEAD 和其他已暂存内容保持不变。桌面与 390px 提示及按钮截图已人工核对，文字完整可见。

既有 `test:git-workbench-diff-prefixes` 连续三次通过，脚本及立即检查行可选的严格断言均未修改；`test:git-workbench-discard-menu` 通过。脚本语法与 `git diff --check` 通过，修改的代码文件均少于 700 行。本轮未改后端，未重跑完整旧浏览器测试链；既有 `conflict-errors` 等待不稳定问题不在本轮范围。

本轮 Chrome 扩展具名后台会话探测返回 `unsupported Codex auth method: apikey`，且浏览器列表为空，因此降级到独立临时 profile 的无头 Chromium。没有接管普通标签、激活 Chrome 或使用有头模式；未访问正在运行的 `:4317` 实例或用户真实仓库。临时 Git 仓库、测试后端、Vite 与浏览器均已清理，5173 端口无监听。构建日志、三次前缀回归结果、浏览器截图与量化结果、扩展失败原文保存在 `/Users/fjh/code/harness/data/runs/Y_sgd0mNlRXp/review-r5-20260916/`。

## 冲突门禁、写后刷新与说明可见性修复（2026-09-16）

本轮处理审查 `1Ytj0A98L0hh/round-6` 的 R6-1 至 R6-4。差异勾选单独接收禁用状态，恢复冲突、未完成操作、只读和忙碌时的行、块及清除选择门禁。差异结果记录对应的操作修订号；写操作期间暂停差异读取，写后旧差异保持可读但不能再次勾选，直到读取到本次操作后的差异，避免连续暂存时选中已经消失的旧块。普通后台刷新仍允许本地勾选，相同内容保留勾选，写入按钮仍等待最新差异。

差异内容变化时，在渲染新差异前清空旧行号，并显示「差异内容已更新，原有勾选已清除，请重新选择」，重新勾选后消除该提示；刷新中的说明明确内容变化时需重新勾选。按项目、工作树、文件及来源切换仍重置局部状态。逐行语义说明与操作按钮移入差异滚动区之外的底栏，说明在按钮上方，长差异下不再需要单独滚到说明处。

浏览器夹具原有的固定诊断 `output` 会覆盖新的底栏，在首次完整链的手机丢弃操作中拦截点击；已改为视觉隐藏且不接收指针事件，DOM 文本保留给原有通知断言。生产页面反馈及原主流程、冲突门禁、特殊前缀测试断言均未改动。

`npm -w web run build` 通过，含类型与约定检查，仅有既有打包体积提示。最新版 `npm -w web run test:git-workbench` 完整八条串行浏览器回归全部通过，成功日志为 `logs/full-8-chain-final.log`。扩展后的刷新专项还独立通过：挂住写后差异响应时行、块、清除选择和写入按钮禁用，放行相同差异后保留原勾选；外部更新替换已勾选的旧行时，旧行消失、勾选清空、写入禁用且有可见解释，重新选择后解释消除，期间没有写请求。桌面和 390px 的 40 行连续修改案例无需滚动到说明处，说明和按钮均完整位于视口内；截图已人工核对。原有暂存区逐行语义、工作区与 HEAD 保护、切换来源、过时响应和历史删除文件路径回归继续通过。

特殊前缀专项在完整链内及两次独立复跑中连续三次通过，原严格可选性断言保留。`test:git-workbench-design` 七视图检查通过。脚本语法及 `git diff --check` 通过，本轮修改的代码文件均少于 700 行；未改后端或平台分支。

本轮先探测 Chrome 扩展，浏览器列表读取失败并返回 `unsupported Codex auth method: apikey`，因此无法创建具名后台会话，随后采用独立临时 profile 的无头 Chromium。没有激活、接管或直连用户 Chrome，没有使用有头模式，也未访问线上 `:4317` 或用户真实仓库。临时浏览器、Vite、fixture 后端及临时仓库均已清理。通道记录、构建日志、首次失败及最终通过日志、截图与尺寸证据保存在 `/Users/fjh/code/harness/data/runs/Y_sgd0mNlRXp/review-r6-20260916/`，验证汇总为该目录下的 `verification-summary.md`。

## 忙碌时只读差异与丢弃成功反馈修复（2026-09-16）

本轮仅处理审查 `1Ytj0A98L0hh/round-7` 的 R7-1、R7-2。写操作期间仍发起所选文件的差异读取，读取结果可浏览，但操作修订号记为尚未就绪；操作结束并重新读取最新差异后才恢复勾选与写入。实际写入同时校验仓库版本和操作修订号，冲突、只读与忙碌门禁保留。

丢弃确认增加仅成功时触发的回调，清理该差异组件的勾选及失效提示，避免主动丢弃成功后要求用户重新选择。取消确认或操作失败不会触发回调；外部变更引起的勾选失效仍显示原有说明。

`npm -w web run build` 通过，包含类型与约定检查，仅有既有打包体积提示。新增 `test:git-workbench-busy-discard` 并接入工作台总入口，完整九条串行回归通过，日志为 `logs/full-chain-initial.log`。专项挂住真实写请求并切换文件，验证请求尚未放行时已读出真实差异，勾选与写入仍禁用；丢弃测试在同一文件保留其他未暂存行，分别验证普通返回和新差异延迟返回都不会误报勾选失效。取消与注入失败保留原勾选；确认框打开后外部改动引起的真实 409 保留磁盘内容和外部更新提示，HEAD、暂存区及其他文件不受丢弃操作影响。忙碌时预览、成功后反馈与失败反馈截图已核对。

特殊前缀专项在完整链内及两次独立复跑中连续三次通过，原有立即检查行可选的断言未改。进一步加强的忙碌专项等待全局操作锁释放后再断言局部差异仍不可勾选，直到最终重读放行，复验通过（`logs/busy-discard-postbusy-final.log`）。原刷新专项在写后场景挂住全部匹配差异请求，避免额外的忙碌期读取绕过延迟条件；其余场景和断言保留，最新版独立复验通过（`logs/refresh-paths-strengthened.log`）。本轮未改布局、后端或平台分支。

本轮重新探测 Chrome 扩展，浏览器列表读取失败，原因为 `unsupported Codex auth method: apikey`，无法创建具名后台会话；随后使用独立临时 profile 的无头 Chromium。没有激活、接管或直连用户 Chrome，没有使用有头模式，也未访问线上 `:4317` 或用户真实仓库。通道记录、构建日志、专项和全链日志及截图保存在 `/Users/fjh/code/harness/data/runs/Y_sgd0mNlRXp/review-r7-20260916/`。

临时浏览器、Vite、fixture 后端及临时仓库均已清理，验证汇总为该证据目录下的 `verification-summary.md`。脚本语法与 `git diff --check` 通过，本轮修改的代码文件均少于 700 行。
