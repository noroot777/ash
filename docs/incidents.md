...
## 裸坐标点击：绕开浏览器规范的隐蔽路径（2026-09-07）

对应约定：根 `AGENTS.md` 的「GUI 自动化按元素点，不按屏幕坐标」。

任务 `KyF1QpKbYWeq`（SDK 57 iOS 模拟器验证）里，codex 用 `osascript` 按屏幕坐标点 Simulator 窗口。两个模拟器窗口重叠之后，有一次点击落到了下面那个 Chrome 窗口上——只造成焦点切换和一次点击，没有新建标签、没有输入文本、没有读取页面内容，agent 发现后停用裸坐标、改走 XcodeBuildMCP 的手势 API。

值得记的不是这次的后果（很轻），而是**它是怎么绕过浏览器规范的**：那一轮从头到尾没有任何浏览器验证，agent 也确实没打算碰 Chrome，所以「不得接管/复用/直连用户的普通标签」这条在它自己看来根本不适用——违反发生在一次它认为与浏览器无关的操作里。

裸坐标点击把「操作 A 应用」和「操作屏幕上恰好在那个位置的东西」画上了等号。一旦窗口层叠关系跟预期不一致，点击落到谁身上就是运气决定的，而 Chrome 恰好是这台机器上最常开着、面积最大的那个窗口。`osascript` 的 `click at {x, y}` 对目标没有任何校验，成功和点错的返回值完全一样，所以它**错了也没有声音**——跟 `HeaderHeightContext` 拿错实例是同一类故障。

判据因此不是「我这一轮碰不碰浏览器」，而是「这次点击的目标是由元素定位决定的，还是由坐标决定的」。模拟器有 XcodeBuildMCP 的手势/AX 通道，浏览器有扩展和 CDP，都能按元素点；裸坐标是最后手段，用之前先确认没有元素通道可走。
