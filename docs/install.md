...
| 依赖 | 要求 | 说明 |
|---|---|---|
| **macOS 或 Linux** | — | 脚本用到 bash/lsof/pkill。Windows 请在 **WSL2** 里跑 |
...
# 或
npm run restart      # 构建 + 后台常驻,日志 /tmp/harness-4317.log
```
...
**常驻**:`npm run restart` 用 detached spawn 起进程,关掉终端也活着。想要开机自启就自己包一层 launchd(macOS)/ systemd(Linux),`ExecStart` 写 `node <仓库>/server/dist/index.js`,`WorkingDirectory` 写仓库根。
...
| 起不来,说端口被占 | 已经有一个实例在跑:`lsof -nP -iTCP:4317 -sTCP:LISTEN` |
