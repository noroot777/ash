...
- **首启向导**:模式未定时首次访问进向导页。选「自用」→ 直接进,保持现状零鉴权;
  选「多人」→ 填管理员姓名 + 根目录路径 + 选一档 **CLI 额度**(§八之二)→ 生成
  管理员 key 整屏展示一次,强制点「我已保存」→ 进登录页。
- **自用 → 多人**:设置页危险区提供转换,流程同上(姓名 + 根目录必填)。
- **多人 → 自用:永不提供**,界面写明原因(多人数据无法合并回单人)。
- 锁死的只有 `instanceMode` 和 `rootDir` 两项;同屏问的 CLI 额度**随时能改**
  (§八之二),别把它跟前两项的单向语义混为一谈。
...
## 八、数据归属:全部按人隔离;宿主机 CLI 订阅默认抹去(可选共用,见 §八之二)

按用户的原则「系统设置里的功能仅对此人可见/生效」,资源一律按人:

- **个人面(每人自己的,互不可见)**:供应商(**不做共享池**,每人自带 API key)、
  执行器、执行模式、起手式、审查者、团队预设等资源条目;默认规则(默认起手式;
  worktree 默认 2026-09-14 搬去了项目级 `projects.use_worktree_default`,不再按人存);
  随手记(转私有);我的 key;个人 CLI 环境(第九节)。
...
**宿主机 CLI 订阅默认抹去,不让任何用户烧**(以下五条是**隔离档**的行为;实例选了
「共用宿主机 CLI」时前四条整体让位,见 §八之二。git 署名那条两档都做):
...
## 八之二、CLI 额度:隔离(默认)/ 共用,一个开关(用户 2026-08-31 拍板)

「每人自带 key」不是唯一的真实场景:**几个人合用一份官方订阅**(一个 Claude Max、
一个 ChatGPT Plus)本来就是常见的用法,把它一刀切掉是过度设计。所以第八节那套抹去
动作整体挂在一个实例级设置上:`AppSettings.sharedHostCli`(默认 `false` = 隔离)。

| | 隔离(默认) | 共用 |
|---|---|---|
| 个人 CLI 配置目录注入 | 注 | **不注**,用宿主机 `~/.claude`、`~/.codex` |
| server 出站凭证清理 | 清 | **不清** |
| 仅 relay CLI 可派发 / `providerId` 必填闸 | 拦 | **放行** |
| 型号探测抑制 | 抑制 | **照常** |
| §九 个人 CLI 环境(技能/全局指令/插件) | 生效 | **整体失效**(目录还在,CLI 不读) |
| git 署名注入 | 注 | **照注** |

最后一行是这一档的要害:**共用额度不等于共用身份**。钱从哪出跟提交署谁的名是两件
事,所以 `GIT_AUTHOR_*` / `GIT_COMMITTER_*` 两档都按 `ownerUserId` 注入。

反过来,§九 那一层在共用档下**必然失效**且无法伪造:claude 的钥匙串查找按配置目录
隔离(§九探针①),要用宿主的登录态就只能用宿主的配置目录,而技能/全局指令/插件都
长在配置目录里。所以设置页那一节直接摆明「现在不生效」,不假装能用。

**两个入口**:

- **首启向导**:转多人的表单里问一次(`web/src/auth/HostCliChoice.tsx`,与设置页
  共用同一份文案与控件)。默认隔离——选错了顶多是「还得各自配供应商」,默认成共用
  则意味着每个新成员一进来就能烧宿主订阅,一个没留神的选择变成了默认行为。
  向导里那条「转换后这些执行器派不出任务」的红色警告只在隔离档下出现:选了共用,
  那些执行器照跑不误,列出来是假警报。
  **补做首启**(上次转换崩在半路)那一屏不问:额度上次已经定过,再问一遍等于把它
  悄悄改掉,所以 `POST /auth/setup` 只在真正的首次转换时接受这个字段。
- **设置 → 默认规则 → 实例模式**:管理员随时能改,**不锁死**(与 rootDir 相反)。
  它是实例面的键,`patchSettingsFor` 现成的管理员闸已经把普通成员挡在外面。

**改档会动到会话的落盘位置**,这是唯一一处需要额外机制的地方:配置目录一换,盘上
已有的 transcript 一个字节都没动,再拿现算的目录去 `--resume` 必然扑空——那正是
2026-08-29 接力事故那句 "No conversation found with session ID"。处置是把「文件当初
写在哪」变成**记录下来的事实**而不是现算的推论:

- `sessions.cli_config_dir` 在起跑时钉住当时注入的目录(`""` = 宿主机默认目录,
  `null` = 这一列上线前的老行——按**当时**那条规则解释:那时多人模式没有「共用」这
  一档,所以 `run_owner_user_id` 非空 = 一定注了个人目录,为空 = 宿主机默认目录。
  **不能按当前设置现算**,也不能回落到任务的 `ownerUserId`(存量任务在转换时整体划给
  管理员,那个字段对「当初写在哪」没有证明力)。
- 续跑前比对「当初那个目录」和「这一轮会去的目录」,不一致就走既有那条优雅路径:
  **另开一条会话行 + 在旧会话时间线里说明原因**(`HOST_CLI_SWITCH_SESSION_NOTE`),
  不是硬失败。这条闸与「换个人回复别人的任务」共用同一个判据
  (`sessionResumableHere`),四条起跑路径(fresh / resume / duet / team 常驻台)
  都过它。

判据函数:`isHostCliIsolated()` / `isHostCliShared()`(`server/src/auth/mode.ts`),
同步路径(`spawn.ts`、`skills.ts`)读镜像 `isHostCliIsolatedSync()`。
**别再拿 `isMultiUser()` 当隔离判据**——那两件事从这一节起分家了。
回归测试:`npm -w server run test:handoff-cli-config-dir` 第 ⑥ 段(翻档、复跑、翻回)。
...
- **实例选了「共用宿主机 CLI」(§八之二)时这一整节不生效**:那一档压根不注入个人
  配置目录,CLI 读的是宿主机那份。目录原样留在盘上(改回隔离档立刻恢复),设置页
  那一节顶部如实标出来,不假装能用。
...
生成管理员 key 展示一次,此后刷新即要求登录。向导同时问一次 **CLI 额度**(§八之二,
默认隔离);**选了隔离才盘点存量执行器**:未挂供应商的逐个列出并警告「多人模式下
不可派发,需先挂供应商」(宿主订阅被抹去,第八节)——选了共用则不列,那些执行器
照跑不误。
...
6. 个人 CLI 环境:配置目录初始化/seed(**先过零交互冒烟测试**,见第九节探针记录)、
   ...
   skill 扫描按人、「个人 CLI 环境」设置节;第 6 批的四道动作全部挂在
   `sharedHostCli` 上(§八之二),含首启选项、设置页开关、`sessions.cli_config_dir`
   与改档后的会话续跑处置。
