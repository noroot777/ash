...
审计基线：`c3d37d3`（`web/src` 对 `web/src`）
...
- 从 `web/src` 的顶层组件和子目录逐个映射到 `web/src` 的 `task-detail/`、`workspace/`、`overlays/`、`debate/`、`team/`、`composer/`、`settings/`、`review/`。
- 对入口类能力额外比对：斜杠命令、命令面板、菜单、快捷键、拖拽、粘贴附件、任务派生、队列、分组、停止/暂停、辩论接力、审查、归档和设置。
- 不把“新版 API client 里有方法”视为完成；必须存在可达 UI 和正确调用语义。
- 用 `git log -S` 和 web 历史判断来源。除特别标注外，下列缺口在 web 历史中都没有出现过完整 UI；不是“有证据的有意砍掉”。
- 审计开始时 `/team`、`/debate` 仍在修；审计过程中 `c3d37d3 feat(web): add task derivation commands` 已把命令菜单、配置卡和来源上下文接入当前分支，因此不再计入未解决总数。
...
- 审计时新版现状：**半残**。`web/src/composer/ComposerFields.tsx:52-59`、`web/src/task-detail/TaskInspector.tsx:174-175` 和 `web/src/debate/DebateHandoff.tsx:41-43` 都直接列出全部 `AGENT_TYPES`；`web/src/composer/TaskComposerPanel.tsx:217-220` 的 `canSubmit` 只检查正文和 busy。用户可创建本机必然跑失败的任务，或把不支持常驻会话的类型选成团队调度者。
- 历史判断：旧版注释记录这套约束曾被多轮审查抓出真实故障；web 的数据层已有 `detectAgents()` 和 `resident` 字段（`web/src/lib/api.ts:345-348`），但主 composer、Inspector、辩论接力没有消费。
- 修复入手点：把旧版 detection/pickable/resident 逻辑抽到 shared 或 web lib；所有执行器选择器共用同一候选生成和提交门禁，探测失败与“确实一个都没装”必须区分。
- 修复结果：新增 `web/src/lib/agentAvailability.ts` 统一缓存探测、生成本机可用类型/显式 Profile/resident 调度者候选，并接入主 Composer、任务 Inspector、派生任务 Composer 和辩论接力。探测中显示进度，探测失败降级为提示但不按“未安装”拦截，确认零 CLI 且零 Profile 时显示空态并统一拦截按钮与提交函数。
...
- 新版现状：**半残**。`web/src/task-detail/TaskInspector.tsx:219-226` 只读展示调度并跳旧版，辩论页完全没有入口。API wrapper 已存在 `schedule/setSchedule/clearSchedule`（`web/src/lib/api.ts:378-385`）和 `fireTask`（`web/src/lib/api.ts:272-273`），但 UI 无调用。
- 历史判断：`f363b93 feat(web): 单任务详情页` 只迁入读取；没有后续编辑提交。
- 修复入手点：迁移 `ScheduleControl` 为 web 组件，在普通任务 Inspector 和辩论配置卡复用；Cron 保留“全新一轮、非续跑”的 `fireTask` 语义。
- 修复结果：新增共享 `web/src/components/ScheduleControl.tsx`，支持读取、设置一次性/Cron、清除、重新启用已执行的一次性定时，并接入普通任务 Inspector 与辩论配置卡；Cron 的“立即触发”直接调用 `fireTask`，提示其为不接续旧会话的全新一轮。
...
- 新版现状：**缺失**。`web/src/composer/TaskComposerPanel.tsx:414-425` 只有“仅创建 / 创建并运行”。
- 历史判断：旧版 `f8b22ac` 已实现；`4feb578` 的 web composer 没有迁入，Git 历史中的 `docs/web-gaps.md` 也曾明确列为存量。
...
- 新版现状：**入口未接线**。`web/src/team/ReviewEvidence.tsx:75-83` 只能查看已有记录或空态；`web/src/task-detail/TaskInspector.tsx:229-236` 也只读摘要。API 已有 `dispatchTaskReview`（`web/src/lib/api.ts:301-305`），全树无 UI 调用。
- 历史判断：旧版 `c9f1e55 feat(web): add independent task review UI`；web 从未迁入。
...
- 新版现状：**缺失且有语义风险**。`web/src/task-detail/TaskHeader.tsx:31-34` 把 failed 映射为立即 retry、canceled 映射为 run；没有回队列等待的动作。API wrapper 已有 `requeueTask`（`web/src/lib/api.ts:270-271`），但无 UI 调用。
...
- 新版现状：**半残**。`web/src/task-detail/TaskInspector.tsx:219-225` 只把 `resumePrompt` 放进只读 `<pre>`。
- 历史判断：web `f363b93` 读取了字段，但没迁编辑；旧版检查点功能由 `541478f`/`749db47` 建立。
...
- 新版现状：**半残**。`web/src/team/TeamReviewWorkspace.tsx:153-175` 只列前 6 个提交和前 8 个文件，明确提示回旧版；没有逐行 diff。单任务 `TaskReviewWorkspace` 已有完整 diff 组件，说明不是数据层限制。
- 历史判断：`988390b` 迁入团队验收时只做摘要，之后未补齐；Git 历史中的 `docs/web-gaps.md` 也曾记录。
- 修复入手点：复用/抽取 `web/src/review/TaskReviewWorkspace.tsx:29-238` 的文件 rail、分段解析和渐进加载，在团队每个 `ReviewRecord` 中展示完整 diff。
...
- 新版现状：**半残**。`web/src/debate/DebateHandoff.tsx:95-108` 一旦找到一个关联团队就只显示该团队卡片，不再允许创建第二组；`web/src/team/TeamHeader.tsx:128-151` 没有再辩入口。API wrapper 已有 `iterateTeamDebate`（`web/src/lib/api.ts:294-295`）但全树无调用。
...
- 新版现状：**半残**。`web/src/team/TeamFeed.tsx:63-77` 和 `web/src/team/WorkerRail.tsx:47-61` 能看到问题并“我来答”，但没有固定 AttentionBar，也没有转交调度者动作。长团队流中提醒可滚走，且用户必须亲自回答本可由调度者调查的问题。
...
- 新版现状：**缺失**。`web/src/settings/AgentsSettings.tsx:18-90` 只编辑静态模型/强度/速度；`web/src/settings/AgentsSettings.tsx:192` 明文声明供应商和 API Key 由旧版承接。相关 API wrapper 在 `web/src/lib/api.ts:391-410` 已齐全但无 UI 调用。
...
- 新版现状：**缺失**。普通任务 `web/src/task-detail/ReplyBox.tsx:15-80` 和团队 `web/src/team/TeamView.tsx:23-75` 只有立即发送。API wrapper 已有 `scheduledMessages/cancelScheduledMessage`（`web/src/lib/api.ts:386-389`），但无 UI 调用。
- 修复入手点：抽成普通/团队回复框共享的 scheduled-message tray；发送时透传 `sendAt`，待发消息按时间排序并可取消。
- 修复结果：普通任务和团队调度台均可选择未来时间发送；两处复用同一待发托盘，加载并按发送时间排序展示持久化消息，可在发送前取消。

### 12. 普通任务回复不能 `@` 召唤另一个智能体进入同一任务（已修）

- 旧版位置：`web/src/ReplyBox.tsx:65-112,190-203` 探测可用 CLI，提供 `@` 候选，并把选中的 agent 传给 reply API。
- 新版现状：**缺失**。`web/src/task-detail/ReplyBox.tsx:13-23,82-103` 的发送协议只接收 text 和 attachments，不传 `agent`。
- 修复入手点：在普通任务 ReplyBox 加 mention token、键盘选择和目标 chip；候选必须复用“已安装类型”检测，不得列出必然失败的类型。团队调度台继续禁用 mention。
- 修复结果：普通任务回复框支持尾部 `@token` 候选、方向键/回车选择和可撤销目标 chip，发送时透传 `agent`；候选只取统一可用性门禁确认可运行的类型，并与 `/team`、`/debate` 菜单互斥共存。团队回复框没有 mention 入口。

### 13. 删除任务后的 Git 清理失败不能二次确认强制清理（已修）

- 旧版位置：`web/src/DeleteTaskModal.tsx:70-145` 在任务已删但 worktree/分支残留时展示真实 git stderr，并提供显式 `--force / -D` 第二步。
- 新版现状：**半残**。`web/src/task-detail/DeleteTaskDialog.tsx:34-72` 只在首次删除时请求普通清理；失败后 toast，并提示回旧版，没有 `discardTaskWorkspace(force)` 调用。
- 修复入手点：保留对话框进入“任务已删除、Git 残留”第二幕；逐项显示剩余 path/branch 和 stderr，用户再次显式确认后调用 `discardTaskWorkspace(..., force: true)`。
- 修复结果：删除记录后若 Git 普通清理失败，对话框会保留并进入第二幕，按实际残留逐项展示 worktree/分支与原始 stderr；用户可选择保留，或再次确认执行 `--force`/`-D`，强制清理仍失败时继续展示最新逐项结果。

### 14. 来源任务看不到自己派生出去的所有团队/辩论（已修）

- 旧版位置：`web/src/DerivedTaskLinks.tsx:15-46` 在来源任务下方列出全部派生团队/辩论及实时状态。
- 新版现状：**缺失一半方向**。`web/src/components/TaskOrigin.tsx:18-26,97-115` 只支持派生任务回看来源/所属团队；普通来源任务没有“向外”的派生清单。辩论页也只取最新关联团队。
- 修复入手点：迁移 `DerivedTaskLinks` 到普通任务详情；辩论接力区改成关联团队列表而不是单个 `linkedTeam`。
- 修复结果：普通任务详情已在回复区上方列出全部派生团队与辩论，按创建时间倒序展示类型、标题、归档状态和实时任务状态；辩论接力区此前已随 P1-8 改为关联团队列表，本次未重复修改。

### 15. 新建任务不能在创建时设置 labels（已修）

- 旧版位置：`web/src/TaskComposer.tsx:73,347-348,614-621` 维护标签并直接写入 create payload。
- 新版现状：**缺失**。`web/src/composer/TaskComposerPanel.tsx:86-102,227-233` 只有 group 等 state 和 common payload，没有 labels；创建后只能去 Inspector 补填。
...
- 新版现状：**行为退化**。`web/src/overlays/NotesPanel.tsx:64-95` 只在显式保存、切换或关闭时保存；`web/src/overlays/NotesPanel.tsx:170-178` 始终是 textarea。崩溃/刷新前的编辑更容易丢，长笔记阅读性下降。
...
- 新版现状：**半残回归**。`web/src/components/MarkdownBody.tsx:9-26` 只有 GFM、图片预览和一律 `target=_blank` 的链接；tailnet 页面上的 localhost open-local 链接、审查 Markdown 弹层和软换行语义均丢失。
- 历史判断：`e71e35a fix(web): restore markdown and conversation metadata` 名义上“恢复 Markdown”，但实现没有迁入上述处理器；旧版 open-local 还经过 `d259672` 的 tailnet 修复。
...
- 新版现状：**缺失**。`web/src/workspace/WorkspaceShell.tsx:109-115` 只有 Cmd/Ctrl+K；界面仍在空态提示“按 C 新建”的旧习惯，但实际没有全局 C 监听。
...
- 新版现状：**入口退化**。`web/src/workspace/ProjectSwitcher.tsx:67-134` 只有设置、搜索和切换；必须走命令面板。`web/src/overlays/CreateEntityDialog.tsx:18-22` 只收 name/path，不调用 `checkPath`。
...
- 新版现状：**缺失**。`web/src/settings/AgentsSettings.tsx:18-90,94-133` 只支持名称、target、model、reasoning、speed；无法读写 `extraArgs`。
...
- 新版现状：**信息退化**。`web/src/workspace/TaskTree.tsx:114-139` 顶层行主要只有状态、置顶、模式和标题；没有暂停依赖、标签、分组、worktree、队列位置。选中任务后 Inspector 能看一部分，但无法横向扫列表。
...
- 新版现状：**缺失**。`web/src/composer/ComposerFields.tsx:305-314` 只列现有分组；`web/src/composer/TaskComposerPanel.tsx:289-300,414-425` 创建后总是离开 composer。
...
- 新版现状：**入口退化**。主工作区没有等价组件；只有进入 `web/src/settings/ProjectSettingsPanel.tsx:47-52` 才能看到健康状态。
...
- 新版现状：**缺失**。`web/src/styles/workspace.css:11-24` 把侧栏固定为 260px/54px，仅支持整体折叠。
...
- 新版现状：**入口半残**。`web/src/team/TeamHeader.tsx:141-152` 的更多菜单只有复制、下载、旧版和归档；删除只能从 Cmd+K 当前任务命令进入。
...
- **Command Palette 命令清单**：两版 `/scope`、`/git` 注册表一致；新版覆盖当前任务动作、新建任务/辩论/随手记/分组/项目、设置、项目切换、分组运行、跨项目搜索，并额外展示进行中任务和父任务跳转。对应 `web/src/App.tsx:470-511`、`web/src/overlays/CommandPalette.tsx:157-261`。
- **普通任务 `/team`、`/debate` 派生**：审计开始时仍在修；`c3d37d3` 已把命令菜单、实时/定稿配置卡、来源会话上下文、worktree 继承和创建跳转接进 `web/src/task-detail/ReplyBox.tsx:57-190`、`web/src/task-detail/TaskDetail.tsx:165-203`。
