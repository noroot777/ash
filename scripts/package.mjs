...
//
// 收件人拿到之后:tar xzf → node scripts/setup.mjs(见 docs/install.md)。
//
...
say(`▶ 打包 ${REF} (${sha}) → ${tarball}`);
// --prefix:解包后自成一个目录,不会把文件炸进对方的当前目录。
const archived = git(["archive", "--format=tar.gz", `--prefix=${name}/`, "-o", tarball, REF], { stdio: "inherit" });
if (archived.status !== 0) die("✕ 打包失败");

const bytes = statSync(tarball).size;
const mb = bytes / 1024 / 1024;
say(`✓ 完成:${tarball} (${mb >= 1 ? `${mb.toFixed(1)}M` : `${Math.max(1, Math.round(bytes / 1024))}K`})`);
say();
