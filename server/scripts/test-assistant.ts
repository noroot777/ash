...
const { validateAssistantWorkflow, assistantActor } = await import("../src/chat/assistant.js");
...
const service = new ChatService(async (selected, _owner, prompt, signal, _project, options) => {
  ...
  return { text: JSON.stringify({ reply: "这是 ash 使用说明。" + "步骤。".repeat(120), matches: [], task: request === "请实现上传" ? { title: "实现上传", body: "完成用户要求的上传功能并验证" } : null, workflow: request === "搭起手式" ? draft : null }) };
