...
try {
  if (child) {
    ...
    const reviews = await import("../src/page-annotation-review.js");
    assert.equal((await reviews.annotationReviewStatus(taskId)).canReopen, true);
    const release = (await store.getAnnotationBatch(taskId, batch.id))?.review;
    assert(release?.releasedAt); assert.equal(release.roundStatus, "paused");
    assert.deepEqual(release.decisions, [], "release never implies user satisfaction");
    ...
    runs.claimTurn(taskId);
    assert.equal((await reviews.annotationReviewStatus(taskId)).canReopen, false, "claim protects the next-run status gap");
    runs.releaseTurn(taskId);
    const gate = await import("../src/rerun-gate.js");
    gate.beginRerunGate(taskId);
    assert.equal((await reviews.annotationReviewStatus(taskId)).canReopen, false);
    gate.endRerunGate(taskId);
    const reviewPath = `/tasks/${taskId}/annotation-batches/${batch.id}/review/pa-test`;
    assert.equal((await request(reviewPath, { verdict: "satisfied", gen: "new-gen" }, "PUT")).status, 200);
    const confirmed = (await store.getAnnotationBatch(taskId, batch.id))!;
    assert.equal(confirmed.review?.decisions[0].verdict, "satisfied");
    assert.equal(confirmed.review?.releasedAt, release.releasedAt);
    assert.equal(confirmed.batch.items[0].comment, "把按钮加大");
    assert.equal((await request(reviewPath, { verdict: "satisfied", gen: "new-gen" }, "PUT")).status, 200);
    assert.equal((await store.getAnnotationBatch(taskId, batch.id))?.review?.decisions[0].savedAt, confirmed.review?.decisions[0].savedAt);
    assert.equal((await request(`/tasks/other-task/annotation-batches/${batch.id}/review/pa-test`, { verdict: "satisfied", gen: "new-gen" }, "PUT")).status, 404);
    const nextRound = await store.saveAnnotationBatch({ ...normalized, id: "followup-batch", items: [{ ...normalized.items[0], comment: "新意见" }] }, 1, null);
    assert.equal(nextRound.batch.taskId, taskId);
    assert.equal((await store.getAnnotationBatch(taskId, batch.id))?.batch.items[0].comment, "把按钮加大");
