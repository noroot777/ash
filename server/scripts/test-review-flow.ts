...
const globalPromptCallsites = [
  ...
  // 调度台是两处:开台/接回的那条 prompt,以及所有 stdin 注入共用的 sendToLead。
  ["团队调度台", new URL("../src/team/session.ts", import.meta.url), 2],
...
const teamSessionSource = readFileSync(new URL("../src/team/session.ts", import.meta.url), "utf8");
assert.match(
  teamSessionSource,
