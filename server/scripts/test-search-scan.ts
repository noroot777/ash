...
const { notes, projects, tasks } = schema;
...
try {
  ...
  // 随手记也在这条时间轴上：它比任务新就该排在任务前面。「任务在随手记前」是相关度档
  // 的产品分区，不是时间轴的一部分 —— 开关上写着「最近更新」，列表却把刚写的随手记压到
  // 几天前的任务下面，那就是在骗人（第 1 轮审查 P2）。
  await db.insert(notes).values([
    { id: "note-sigma-new", projectId: "project", body: "随手记里也提到了 sigma", attachments: null, ownerUserId: null, createdAt: Date.parse(iso(900)), updatedAt: Date.parse(iso(900)) },
    { id: "note-sigma-old", projectId: "project", body: "很旧的一条 sigma 随手记", attachments: null, ownerUserId: null, createdAt: Date.parse(iso(3)), updatedAt: Date.parse(iso(3)) },
  ]);
  assert.deepEqual(
    (await searchAll("sigma")).map((hit) => hit.id),
    ["recent-title-old", "recent-convo-new", "note-sigma-new", "note-sigma-old"],
    "相关度档：任务整体在随手记之前",
  );
  assert.deepEqual(
    (await searchAll("sigma", { sort: "recent" })).map((hit) => hit.id),
    ["recent-convo-new", "note-sigma-new", "recent-title-old", "note-sigma-old"],
    "最近更新档：任务和随手记按同一条时间轴混排",
  );
  ...
  // 早停等价：最新的那条只在会话里命中，它必须排在所有标题命中之前 —— 这正是相关度档
  // 那套「会话档整堆先不读」在这一档会漏掉的人。随手记也占前 50 名的位置，所以早停的
  // 判据是「第 50 名有多新」而不是「收了几条任务命中」。最旧的仍被上限挤掉。
  await db.insert(tasks).values([taskRow("recent-convo-top", "只在会话里(最新)", "", 1100)]);
  writeRun("recent-convo-top", "这段会话里 alpha 和 beta 都出现了。");
  await db.insert(notes).values([
    { id: "note-alpha-top", projectId: "project", body: "alpha 和 beta 都在这条随手记里", attachments: null, ownerUserId: null, createdAt: Date.parse(iso(1200)), updatedAt: Date.parse(iso(1200)) },
  ]);
  const cappedRecent = await searchAll("alpha beta", { sort: "recent" });
  assert.equal(cappedRecent.length, 50, "上限仍是 50 条");
  assert.deepEqual(
    cappedRecent.slice(0, 3).map((hit) => hit.id),
    ["note-alpha-top", "recent-convo-top", "alpha-other"],
    "最新的三条依次是随手记、只在会话里命中的任务、别的项目的任务 —— 三种「相关度档会压后」的东西",
  );
  assert.ok(
