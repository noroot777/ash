...
export async function assistantFormatter(room: Room): Promise<typeof chatPrompt> {
...
export async function invokeAssistant(member: ChatMember, room: Room, prompt: string, signal: AbortSignal, invoke: typeof invokeChat) {
  ...
  for (let round = 0; round < 3; round++) {
    ...
    const actor = await assistantActor(room.ownerUserId);
    const response = await invoke(member, room.ownerUserId, prompt + evidence, signal, room.projectId, { purpose: "assistant" });
    const raw = parseLastJsonObject(response.text);
