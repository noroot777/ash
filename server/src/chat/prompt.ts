...
查询优先使用 Read、Glob、Grep 等直接读取工具；需要 shell 时只用单条 cat、head、tail、ls、pwd、wc、stat、grep 或基础 rg，不使用脚本、管道、重定向、复合命令。ash 会检查每个工具事件，检测到写入或无法确认只读时终止回复并持久显示警告；咨询期间的目录变化会随回复如实附注。
