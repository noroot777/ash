...
export const traeSpec: CliSpec = {
  ...
  notes:
    ...
    "②`-y/--yolo`「YOLO 模式,跳过工具权限检查」= 自动批准,不给就会卡在交互确认(ash 里表现为任务永不结束)。" +
    ...
    "⑤`--session-id`「使用指定的会话 ID 进行会话跟踪」+ `--resume`「通过 ID 恢复一个会话」,按 ash 自发 id 那一档接。" +
    ...
    "⑧`-w/--worktree` 是 TRAE 自己开 git worktree,**绝不能带**——ash 已经自己管 worktree,两套会打架。" +
    "仍未确认(装上后按这个顺序测):①`--session-id` 收不收 ash 发的随机 UUID,以及它和 `--resume` 是不是同一个 id 空间——" +
