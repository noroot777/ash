// 验收那一刻要动的 git —— 把任务分支合进目标分支。
//
// 从 git.ts 拆出来的（那边只剩仓库探测与 worktree 生命周期）：合并这一块自成一体，
// 而且它是唯一会写「共享状态」的地方（目标分支的 ref、项目工作区），单独一个文件读
// 起来才看得清「哪几条路会碰用户的工作区」。合并之后的清理（删 worktree、删分支）在
// git-accept-cleanup.ts —— 那边失败时合并已经发生且不可撤销，退路完全是另一套。
//
// 怎么合由**线上那一站**说了算（AcceptStrategy），调用方在 task-accept.ts 里从任务的
// 工作流快照读出来传进来；不传就是老规矩（安全合并）。
import { resolve } from "node:path";
import { realpathSync } from "node:fs";
import type { AcceptStrategy } from "@ash/shared/workflow";
import {
  expandHome,
  gitError,
  isGitRepo,
...
import { checkoutRecovery, registeredCheckout, removeMissingWorktreeRegistrations } from "./git-worktree-state.js";
...
// ── Deterministic acceptance merge ─────────────────────────────────────────
