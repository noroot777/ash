...
// 删除中断可能留下 .git 文件，却已经没有它指向的登记。先验证剩余源码，再整目录备份，
...
export async function preserveRemovedWorktree(repo: string, path: string, branch: string): Promise<string | null> {
  ...
  if (!directory?.isDirectory() || !(await lstat(pointer).catch(() => null))?.isFile()) return null;
  ...
  try {
    ...
    await exec("git", [...args, "read-tree", `refs/heads/${branch}`], options);
  ...
  await rename(join(backup, ".git"), `${backup}.git-pointer`);
  return backup;
}
