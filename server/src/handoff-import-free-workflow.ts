...
//     的 reviewerName 文本(展示够用),预约不落地(armed 强制为 false),否则会预约一个
//     本机根本不存在的审查者,到点起跑当场失败。
...
export async function buildFreeWorkflowRows(
  ...
  const state = payload.state
    ? {
        ...
        reviewArmed: payload.state.reviewArmed && (!wantedName || !!reviewerId),
        ...
        reviewRunId: payload.state.reviewRunId && runIds.has(payload.state.reviewRunId)
