...
import { recoveryDiff } from "./task-base-diff.js";
import { commitAt, containsCommit } from "./task-branch-plan.js";
import { execFileText as exec } from "./exec.js";

export async function manualBaseProposal(repo: string, current: string | null,
  ...
  if (current) {
    ...
    try {
      const preview = await recoveryDiff(repo, start, current);
      return { start, manual: { basis, ...preview }, blocker: null };
    } catch {
      return { start, manual: { basis, files: [], fileCount: 0, diff: "", truncated: false }, blocker: "恢复差异暂时无法读取，请刷新重试；未解除挂起或修改当前代码。" };
    }
  }
  for (const [commit] of candidates) { if (commit && await commitAt(repo, commit)) { start = commit; break; } }
  return { start, manual: { basis, files: [], fileCount: 0, diff: "", truncated: false }, blocker: null };
}
