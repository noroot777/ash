...
//
// 一条线是这样跑起来的：几个「会停下来等」的锚点（干活等 agent、自动验证等审查任务、
...
//   ② 一段跑砸了就地停下，**不往下推**：别在一条卡住的线上派审、也别送进人工关口。
...
/** 这一站已经派过几轮审查（按站数，不是这个任务被审过的总次数）。 */
async function stationRounds(taskId: string, stepId: string): Promise<number> {
  const rows = await db
    .select({ id: tasks.id, reviewStep: tasks.reviewStep })
    .from(tasks)
    .where(eq(tasks.reviewOf, taskId));
  // 老审查任务身上没有 review_step（那时一条线只可能有一站验证）：把它们算进第一站。
  return rows.filter((row) => (row.reviewStep ?? stepId) === stepId).length;
}
...
async function atVerifyStation(task: TaskRow, def: WorkflowDef, station: WorkflowStep): Promise<boolean> {
  ...
  // 先挪游标再派审：派审那一步要读游标才知道该用哪一站写的执行器去验。
  await setWorkflowAt(task.id, station.id);
  const { dispatchReviewTask } = await import("./review.js");
  await dispatchReviewTask(task.id).catch((error) => appendTaskTimeline(
    task.id,
    `想在「${STEP_LABELS.verify}」这一站派审，但没派出去（${error instanceof Error ? error.message : String(error)}）。`,
  ));
  return false;
}
