...
/**
 * 「思考强度」自己的一颗胶囊。它**不再是选模型下拉里的第二步**：模型和强度是两件
 * 独立的事——换模型不该顺手把强度改掉，改强度也不该逼你重走一遍选模型。所以所有
 * 选模型的地方统一是两颗：前面「执行器 · 模型」，后面「思考强度」。
 *
 * 代价是两颗之间可能对不上（`gpt-5.5` 配 `ultra`），这时**明说而不是静默改掉**：
 * 胶囊转成警告态、浮层顶上写清楚为什么，用户自己决定是换模型还是换档位。静默清空
...
export function EffortPicker({
  ...
  return (
    <div className={`effort-picker is-${variant}`}>
      ...
      {/* chip 变体挤在对话框底部那一排里，多一行字会把工具条撑变形：那里靠红色警告态
          加浮层里的说明。 */}
