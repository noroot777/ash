...
// 应用内的悬停提示。原生 `title` 被约定禁掉（`web-next/CLAUDE.md`），而「一颗纯图标按钮，
...
export type HoverTipAnchor = { x: number; y: number };
...
export function HoverTip({ at, children }: { at: HoverTipAnchor | null; children: ReactNode }) {
  if (!at) return null;
  return createPortal(
    <span className="ui-hover-tip" role="tooltip" style={{ left: at.x, top: at.y }}>{children}</span>,
    document.body,
  );
}
