 ...
 * ③ portal 会脱离触发器原来的堆叠上下文；触发器若在高层对话框里，浮层层高必须跟着
 *    超过那层，否则坐标虽对，画面却会被对话框遮罩盖住。
 */
...
export interface PanelPlacementOptions {
  ...
  /** 再挤也要留出的高度。 */
  minHeight?: number;
  /** portal 浮层在普通页面上的基础层高。 */
  baseZIndex?: number;
  /** 传 false 就完全不算（内联渲染的那份浮层用）。 */
  enabled?: boolean;
}

function panelZIndex(anchor: HTMLElement, baseZIndex: number): number {
  let zIndex = baseZIndex;
  for (let element: HTMLElement | null = anchor; element; element = element.parentElement) {
    const ancestorZIndex = Number.parseInt(window.getComputedStyle(element).zIndex, 10);
    if (Number.isFinite(ancestorZIndex)) zIndex = Math.max(zIndex, ancestorZIndex + 1);
  }
  return zIndex;
}
...
export function usePanelPlacement(
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      setPlace({
        ...
        zIndex: panelZIndex(anchor, baseZIndex),
      });
    };
    measure();
  ...
  }, [anchorRef, baseZIndex, enabled, fallbackHeight, gap, minHeight, minWidth, panelRef]);
