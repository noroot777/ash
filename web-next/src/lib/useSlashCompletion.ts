...
import type { SkillEntry } from "@harness/shared";
...
//
// 只做补全：选中一条就是把 `/名字 ` 写进正文，harness 不改写、不截走。
export function useSlashCompletion({
  ...
  const items = disabled || dismissed ? [] : mergeSlashItems(harness, skills, slashToken(value));
