...
function Demo() {
  ...
  const returns = useRemoteReturns(setToast);
  ...
  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column" }}>
      ...
      {visible && (local ? <p role="status">本机任务：{local.title}</p> : (
        <RemoteTaskDetail archive={selected} target={target} returns={returns} onLocalOwnership={onLocalOwnership} notify={setToast} />
      ))}
