...
try {
  ...
  assert.match(
    await page.locator(".duet-gate-uploading").innerText(),
    /上传中 \d+% · 传完才能放行或打回/,
    "收起输入区后要写明为什么现在放不了行",
  );
  ...
  // ③ 传完之后照常放行；已经传好的那张在输入区收起时也看得见。
  await release("g1.png");
  await page.locator(".task-upload-chip img").waitFor();
  assert.equal(await page.locator(".duet-gate-uploading").count(), 0, "传完不该还挂着上传中提示");
  assert.equal(await approve.isDisabled(), false, "传完必须能放行");
