...
// 两种写法都得筛得到：`T T` 是显示出来的样子，`tt` 是照着敲进去的样子。
const haystack = (keys) => keysSearchText(keys).toLocaleLowerCase();
assert.equal(haystack(TASK_MODE_SHORTCUT_LABEL).includes("tt"), true);
assert.equal(haystack(TASK_MODE_SHORTCUT_LABEL).includes("t t"), true);
assert.equal(haystack(undefined), "");
assert.equal(haystack("NI").includes("ni"), true);

// 回车直达：整串输入正好是那条的键位才算，大小写和空格都不挑。
assert.equal(matchesKeysQuery(TASK_MODE_SHORTCUT_LABEL, "tt"), true);
assert.equal(matchesKeysQuery(TASK_MODE_SHORTCUT_LABEL, "T T"), true);
assert.equal(matchesKeysQuery(TASK_MODE_SHORTCUT_LABEL, "TT"), true);
assert.equal(matchesKeysQuery("NI", "ni"), true);

// 半截、多打一个字符、以及没有键位的条目都不能被直达劫走。
assert.equal(matchesKeysQuery(TASK_MODE_SHORTCUT_LABEL, "t"), false);
assert.equal(matchesKeysQuery(TASK_MODE_SHORTCUT_LABEL, "ttt"), false);
...
assert.equal(normalizeKeys("T T"), "tt");
