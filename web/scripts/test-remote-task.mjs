...
assert.doesNotMatch(handoff, /handoff-return-source-url/, "移回不再把地址输入框推给用户，地址一律由服务端按来源指纹探测");
assert.doesNotMatch(returnView, /<select|<input(?![^>]*type="checkbox")/, "移回没有任何可选项，不该出现下拉或文本输入");
assert.match(returnView, /连不上来源机器/, "连不上来源机时必须显式提示，而不是停在空白弹窗");
