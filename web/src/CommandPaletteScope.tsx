...
export function scopeProjectName(scope: SearchScope, projects: ProjectView[]): string {
  if (!scope.projectId) return "不限项目";
  return projects.find((project) => project.id === scope.projectId)?.name ?? "未知项目";
}

export function scopeTypeName(type: SearchScopeType): string {
...
export function ScopeToken({
  ...
  return (
    <span className="flex shrink-0 items-center overflow-hidden rounded-md border border-accent/25 bg-accent/10 text-[11px] text-accent">
      <button className="px-2 py-1 font-medium outline-none hover:bg-accent/10 focus-visible:bg-accent/15" onClick={onEdit}>
...
function ChoiceRow({
  ...
  return (
    <button
      onMouseMove={onHover}
      onClick={onChoose}
      className={`flex w-full items-center gap-3 px-4 py-2.5 text-left outline-none ${active ? "bg-overlay" : ""}`}
    >
...
export function ScopeProjectStep({
  ...
  return (
    <div className="py-1">
      <div className="px-4 pb-1 pt-2 text-[10px] font-medium uppercase tracking-wide text-faint">选择项目 · 1 / 2</div>
...
export function ScopeTypeStep({
  ...
  return (
    <div className="py-1">
      <div className="px-4 pb-1 pt-2 text-[10px] font-medium uppercase tracking-wide text-faint">选择类型 · 2 / 2</div>
