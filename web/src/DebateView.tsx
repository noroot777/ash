    ...
    <main className="flex h-full min-h-0 flex-col">
      <header className="border-b border-neutral-800 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <button onClick={onDelete} className="rounded-md border border-neutral-800 px-2 py-1.5 text-sm text-neutral-500 hover:text-red-400">
            删除
          </button>
        </div>
        {cfg && (
          <p className="mt-2 text-xs text-neutral-500">
            辩手A <b className="text-neutral-300">{cfg.debaterA}</b> ↔ 辩手B{" "}
            <b className="text-neutral-300">{cfg.debaterB}</b> · 实现方 辩手{cfg.implementer} · 轮数{" "}
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
        {turns.length === 0 && (
          <p className="text-sm text-neutral-600">点击「运行」开始对抗。双方逐回合的发言会实时显示在这里。</p>
        )}
        {turns.map((t, i) => (
...
function Bubble({ turn, prevRound }: { turn: DebateTurn; prevRound?: number }) {
  ...
  return (
    <div className="mb-3">
      {showDivider && (
        <div className="my-3 text-center text-xs text-neutral-600">
      ...
      <div className={`flex flex-col ${align}`}>
        <div className={`max-w-[88%] rounded-lg border px-3 py-2 ${color} ${side === "impl" ? "w-full max-w-full" : ""}`}>
          ...
          <div className="whitespace-pre-wrap break-words text-[13px] leading-relaxed text-neutral-200">{turn.text}</div>
    ...
    <div className="border-t border-violet-500/40 bg-violet-500/5 px-6 py-3">
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          ...
          <button onClick={() => onGate({ kind: "reject" })} className="rounded-md border border-neutral-700 px-3 py-1 text-xs text-neutral-300">
            打回终止
          </button>
          <button onClick={() => setMode(mode === "inject" ? null : "inject")} className="rounded-md border border-neutral-700 px-3 py-1 text-xs text-neutral-300">
            注入意见→回炉
          </button>
          <button onClick={() => setMode(mode === "ask" ? null : "ask")} className="rounded-md border border-neutral-700 px-3 py-1 text-xs text-neutral-300">
            提问→继续
          </button>
        </div>
      </div>
      {mode && (
        <div className="mt-2 flex gap-2">
          <textarea
            ...
            className="flex-1 resize-none rounded-md border border-neutral-800 bg-neutral-900 px-2 py-1 text-sm outline-none"
          />
          <button
            disabled={!text.trim()}
            onClick={() => {
