...
export function AssistantView({ project, projects, onTask, onSettings, onExit, onMode, onChat }: {
  ...
  return <section className="chat-shell assistant-shell" aria-label="ash 助手">
    ...
    {!configure && <header className="chat-header assistant-header"><div className="assistant-heading">{chat.room ? <AssistantConversationTitle key={chat.room.id} name={chat.room.name} onRename={chat.renameConversation} /> : <h1><AssistantIcon size={23} />ash 助手</h1>}{chat.room && <span className={`chat-connection${chat.connected ? " is-connected" : ""}`} tabIndex={0} role="status" aria-label={chat.connected ? "实时连接" : "连接中，状态可能延迟"} {...connectionTip.anchorProps} />}</div>
