...
import { ArrowLeft, ArrowUp, At, ChatCircleDots, Hash, Plus, Stop, UsersThree } from "@phosphor-icons/react";
...
export function ChatView({ project, onTask, onExit, onMode }: {
  ...
  const candidates = room?.members.filter((member) => member.name.toLowerCase().includes(token.toLowerCase())) ?? [];
  return <section className="chat-shell" aria-label="聊天模式">
    <nav className="chat-rail" aria-label="聊天频道"><header><span className="chat-brand"><ChatCircleDots size={25} weight="fill" />聊天</span><button type="button" aria-label="返回工作区" onClick={onExit}><ArrowLeft size={17} /></button></header><div className="chat-project">{project.name}<small>点名协作 · 安静待命</small></div>
      ...
      <div className="chat-rail-note"><At size={22} /><strong>有需要，再叫上它。</strong><p>没有 @ 的消息只保存在群里，不启动任何智能体。</p></div>
    </nav>
    <div className="chat-main"><div className="chat-mode-bar" aria-label="工作模式">{MODES.map((mode) => <button type="button" key={mode.value} onClick={() => onMode(mode.value)}>{mode.label}</button>)}<button type="button" aria-current="page" className="is-active"><ChatCircleDots size={14} />聊天</button></div>
      {editor ? <ChatMembers key={`${editor}-${roomId}`} creating={editor === "create"} initial={editor === "members" ? room?.members ?? [] : []} onCancel={() => setEditor(null)} onSave={async (members, name) => {
        ...
        else if (roomId) { const updated = await chatApi.members(roomId, members); setSnapshot((current) => current ? { ...current, room: updated } : current); setEditor(null); }
      }} /> : <>
        <header className="chat-header"><div><h1><Hash size={23} />{room?.name ?? "聊天空间"}</h1><p><span className={`chat-connection ${connected ? "is-connected" : ""}`} />{roomId ? connected ? "实时连接" : "连接中，状态可能延迟" : "创建群聊，邀请你的智能体"}</p></div>{room && <button type="button" className="chat-member-count" onClick={() => setEditor("members")}><UsersThree size={19} />{room.members.length} 位成员</button>}</header>
        {snapshot ? <ChatMessages snapshot={snapshot} onTask={onTask} onMention={mention} /> : <div className="chat-empty"><ChatCircleDots size={48} weight="duotone" /><h2>{ready ? "把想法变成对话" : "正在载入聊天…"}</h2><p>建立一个空间，和你的智能体一起聊。</p>{ready && <button type="button" className="chat-primary" onClick={() => setEditor("create")}>新建群聊</button>}</div>}
        {room && <div className="chat-composer-area"><div className="chat-live-line">{active.length ? <><span className="chat-live-dot" />{active.map((message) => message.author).filter((name, index, all) => all.indexOf(name) === index).join("、")} 正在回复<button type="button" onClick={() => void chatApi.stop(room.id).then((result) => { if (selected.current === room.id) setSnapshot(result); }).catch((reason) => setError(String(reason)))}><Stop size={12} weight="fill" />停止回复</button></> : <span>所有成员安静待命</span>}</div>
          <div className="chat-composer">{mentionOpen && candidates.length > 0 && <div ref={mentionMenu} className="chat-mention-menu" role="listbox" aria-label="点名成员">{candidates.map((member, index) => <button role="option" aria-selected={index === mentionIndex % candidates.length} className={index === mentionIndex % candidates.length ? "is-selected" : ""} type="button" key={member.id} onClick={() => mention(member.name)}><span className={`chat-avatar tone-${index % 4}`}>{member.name.slice(0, 1).toUpperCase()}</span><strong>@{member.name}</strong><small>{member.agentType}</small></button>)}</div>}
            <textarea ref={input} aria-label="群聊消息输入" value={draft} maxLength={8000} disabled={sending} placeholder={`发消息到 #${room.name}，输入 @ 选择成员…`} onChange={(event) => { setDraft(event.target.value); setMentionOpen(/(?:^|\s)@[^\s@]*$/u.test(event.target.value)); setMentionIndex(0); }} onKeyDown={(event) => {
