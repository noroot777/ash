...
export function ComposerObjective({ body, mode, textareaRef, onChange, onPaste, items, selected, token, onSelect, onPick, onDismiss, onSubmit, mention }: {
  ...
  onDismiss: () => void;
  onSubmit: () => void;
  /** `@` 引用项目文件那一套：菜单自己画，键盘先给它过一手（返回 true = 已被吃掉）。 */
  mention?: { menu: React.ReactNode; onKeyDown: (event: KeyboardEvent) => boolean };
}) {
  return <div className="composer-objective">
    <textarea ref={textareaRef} autoFocus aria-label="任务目标" value={body} onPaste={onPaste}
      ...
      onKeyDown={(event) => {
        // `/` 菜单开着时它优先（那是刚敲下的第一个字符）；否则交给 `@` 引用。
        if (!items.length && mention?.onKeyDown(event)) return;
        if (items.length) {
