import type { AgentType, SkillEntry, SkillSource } from "@ash/shared";
...
export interface SlashItem {
  ...
  label: string;
  hint?: string;
  /** ash 自己的派生命令(/team、/duet)vs CLI 已装的技能。 */
  kind: "ash" | "skill";
  source?: SkillSource;
...
/**
 * 合成候选:**ash 自己的命令永远排在最前**。它们改变的是「谁来干这件事」
 * (派生一个团队/一场讨论),技能只是给当前这轮加一句提示词 —— 前者点错了代价大得多,
 * 不能让某个 CLI 装了个叫 team 的技能就把它挤下去。命中位置排序只在各自组内进行。
 */
export function mergeSlashItems(ash: SlashItem[], skills: SkillEntry[], token: string | null): SlashItem[] {
  if (!token) return [];
  return [...rankByMatch(ash, token), ...rankByMatch(toSlashItems(skills), token)];
}
