...
export function CreateGroupDialog({ onClose, onCreate }: {
  ...
  return <ConfirmDialog className="create-group-dialog" title="新建分组" eyebrow="WORKFLOW GROUP" icon={<StackPlus size={20} weight="duotone" />} message="并行组会同时启动成员；串行组按队列顺序推进。" confirmLabel="创建分组" busy={busy} onClose={onClose} onConfirm={() => void create()}>
