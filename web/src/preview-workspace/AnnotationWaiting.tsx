...
export function AnnotationWaiting({ controller: c, review, starting }: {
  controller: ReturnType<typeof useAnnotationBatch>; review: ReturnType<typeof useAnnotationReview>; starting: boolean;
}) {
  ...
  return <section className="annotation-waiting" aria-label="修改期等待区">
    ...
    {sent.some((record) => record.state === "reviewable") && review.canPrompt && !c.busy && !c.review && <div className="annotation-reopen" role="status">
    ...
    {!sent.length && <p>尚无已发送批次。可先通过任务的预览入口启动页面，或使用右侧截图批注。</p>}
