...
export function PreviewLauncher({ taskId, preview, refresh }: {
  taskId: string; preview: FreeWorkflowPreviewState | null; refresh: () => Promise<void>;
}) {
  ...
  return <section className="preview-launcher" aria-label="启动页面预览">
    <PreviewLaunchOptions state={preview ? state : { ...state, loading: true }} starting={starting} stopped={preview?.services?.some((s) => s.status === "stopped") ?? false}
      ...
      onStart={(input) => void controller.start(input, !preview || preview.running)} onCancel={() => void controller.cancel()} onRetry={() => void controller.load()} />
...
export function PreviewLaunchOptions({ state, starting, stopped, failed = false, onStart, onCancel, onRetry }: {
  state: PreviewLaunchState; starting: boolean; stopped: boolean; failed?: boolean;
  ...
  return <>
    <h3>{starting ? "正在启动页面预览…" : info?.directory === null ? "无法启动页面预览" : "在工作区启动预览"}</h3>
    <p role="status">{state.loading ? "正在检查任务工作目录与预览候选…" : starting
      ...
      : info?.reason || state.notice || (failed ? "上次预览未能启动或已退出，请查看预览日志后重试。"
        : stopped ? "上次预览已停止，可重新启动，或使用右侧截图批注。" : "选择本次要看的页面，启动后即可浏览和标注。")}</p>
    ...
    {info?.directory && !info.reason && <>
      ...
      {selectedStep && <div className="preview-launch-candidate"><strong>工作流预览命令</strong><pre>{selectedStep.command}</pre>
        <button type="button" disabled={blocked} onClick={() => start({})}>启动工作流预览</button></div>}
      {info.configured && <div className="preview-launch-candidate"><strong>已保存的项目预览配置</strong><pre>{info.configured.command}</pre>
        <button type="button" disabled={blocked} onClick={() => start(info.configured!)}>按已保存配置启动</button></div>}
