...
export function PreviewWorkspaceLayout({ children, notesOpen, onToggleNotes, onClose, onKeyDown }: {
  ...
  useDismissable({ enabled: expanded, containerRef, restoreFocusRef: expandRef, closeOnOutside: false, onClose: () => setExpanded(false) });
  return <section ref={containerRef} className={`preview-workspace${expanded ? " is-expanded" : ""}${notesOpen ? "" : " notes-collapsed"}`}
    ...
    <header className="preview-workspace-header">
      ...
      <div className="preview-workspace-layout-actions">
        ...
        <button ref={expandRef} type="button" aria-pressed={expanded} onClick={() => setExpanded(!expanded)}>
