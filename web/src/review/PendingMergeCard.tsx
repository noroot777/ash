...
// 「合并后不提交」那一档验收完之后的状态卡，**只住在验收台**（每个有验收动作的表面都
// 带着它：单任务验收台、团队验收台、duet、工作流站点）。
//
// 为什么需要它（现场，2026-09-15）：那一档验收完，界面上只剩一句和正常验收一模一样的
// 「✓ 验收完成」，欠的那一步只活在一个 toast（刷新就没）和时间线深处（得翻历史）。用户
// 因此完全看不出还欠一步，也找不到地方收尾，最后手工 merge --no-ff 补进目标分支。
//
// 为什么不在任务对话区也铺一张（用户 2026-09-15 当场定的）：那张卡连「收尾完成」的绿态
// 都会一直占着对话区顶部一整行，看完就没用了。对话区改留 TaskHeader 里一颗「待提交」
// 徽记——刷新后照样看得出还欠一步（根 AGENTS.md「持久可见的状态」那一条仍然满足），
// 点一下就进验收台看这张卡。
//
// 三件事这张卡都自己负责：
