...
export function ReleaseWorkspaceControl({ task, disabled, blocker, onReleased }: {
  ...
  return <div>
    {workspace && !workspace.path && workspace.branch
      ? blocker
        ? <p role="alert" style={{ whiteSpace: "pre-line" }}>父任务工作区目录已不存在，但占用登记尚未解除。{blocker}</p>
    ...
    {error && !proposal && <p role="alert" style={{ whiteSpace: "pre-line" }}>{error}</p>}
    {proposal && <ConfirmDialog title="释放父任务工作区目录？"
      ...
      {error && <p role="alert" style={{ whiteSpace: "pre-line" }}>{error}</p>}
    </ConfirmDialog>}
  </div>;
}
