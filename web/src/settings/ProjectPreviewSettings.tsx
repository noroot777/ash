...
export function ProjectPreviewSettings({ project, onUpdated, notify }: {
  ...
  return <section className="settings-section project-preview"><h2>预览</h2><div className="settings-card">
    <div className="settings-row"><div><b>启动方式</b><small>在这里配置一次，任务里的「打开预览」按保存的方案启动。</small></div></div>
    ...
    {config.mode === "script" ? <>
      <label className="settings-field"><span>启动脚本</span><ScriptEditor label="启动脚本" value={script} onChange={setScript} readOnly={!canManage || busy} rows={9} placeholder={`例如：\ncd web\nnpm run dev -- --port ${variable("PORT")}`} /></label>
      ...
      <small>检测读取已保存的项目目录中的常见启动配置，不运行命令。总启动脚本和子服务可能同时出现，请避免重复勾选。</small>
      ...
      <div className="preview-service-list">{config.services.map((service) => <div className={`preview-service-card${service.enabled ? " is-selected" : ""}`} key={service.id}>
        <div className="preview-service-heading">
          ...
          <button type="button" disabled={!canManage || busy} aria-label={`移除 ${service.name}`} onClick={() => setConfig((current) => ({ ...current, services: current.services.filter((s) => s.id !== service.id), primaryServiceId: current.primaryServiceId === service.id ? null : current.primaryServiceId }))}>移除</button>
        </div>
      ...
      <small>每条脚本都从任务工作区根目录独立执行，使用自己的 <code>{variable("PORT")}</code>。已选服务按列表顺序对应 <code>{variable("URL1")}</code>、<code>{variable("URL2")}</code>…，可传给前端开发服务器的接口代理配置。它们是服务端内部地址。</small>
    </>}
