...
/**
 * 一条回合此刻该怎么排：**它自己还在飞就一律平铺，收口那一下才折**。
 *
 ...
 *
 * 闸只看**这一条回合**结束没有（会话行的 `endedAt`），不看整个任务停没停。任务往下还有
 * 好几站要走 —— 换轮、就地验证、审查门、团队执行者还在干活 —— 拿任务状态当闸，讲完的
 * 回合会一直摊在那儿不收（用户 2026-09-01 反馈：「执行完了怎么没有自动折叠」）。回合是
 * 单调的：`endedAt` 落下来就不会再回到 null，所以折过一次位置就不再变。
 */
