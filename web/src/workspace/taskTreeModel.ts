...
import { hasFailed, needsYourCommand } from "../lib/taskAttention.ts";
...
//
// 五类，理由是同一句话「我要一直看得见」的五种形状 —— 用户手动摁下的记号（星标、置顶）、
// 没盖的章（unaccepted）、坏在半路的活（失败）、以及停在那儿等你指挥的（提问中 /
// 验证没通过，团队时问题写在执行者身上，所以要连 workers 一起看）。
//
